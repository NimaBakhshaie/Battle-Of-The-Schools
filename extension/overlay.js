(() => {
  ...
  let wakeEnabled = false, wakeRecognition, wakeTimer, commandTimer, awaitingCommand = false, wakeCommand = '', speaking = false;
  function loadVoices() {
  ...
  function speak(text) {
    ...
    speech.onend = () => { speaking = false; $('orb').classList.remove('speaking'); scheduleWake(); };
    speech.onerror = event => { speaking = false; scheduleWake(); $('orb').classList.remove('speaking'); if (!['interrupted', 'canceled'].includes(event.error)) $('caption').textContent = 'Voice unavailable: ' + event.error + '. Try another voice or press Preview.'; };
  ...
  function scheduleWake() {
    ...
    if (wakeEnabled && !speaking && !listening && !busy && !document.hidden) wakeTimer = setTimeout(startWake, 700);
  }
  function startWake() {
    if (!wakeEnabled || speaking || listening || document.hidden || busy) return;
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { $('wake').textContent = 'Wake voice unavailable in this browser'; return; }
    const recognizer = new SR(); wakeRecognition = recognizer;
    ...
    recognizer.onresult = async event => {
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        const match = words.match(/\b(?:hello|hey)\s+orbit\b[,.!?]?\s*(.*)/i);
        if (match) {
          awaitingCommand = true; wakeCommand = match[1].trim(); $('card').hidden = false;
          $('status').textContent = 'Listening for your instruction…'; $('orb').classList.add('listening');
          clearTimeout(commandTimer);
          commandTimer = setTimeout(() => { awaitingCommand = false; wakeCommand = ''; $('orb').classList.remove('listening'); }, 15000);
        } else if (awaitingCommand) wakeCommand = words;
        if (awaitingCommand && wakeCommand) {
          const task = wakeCommand; awaitingCommand = false; wakeCommand = ''; clearTimeout(commandTimer);
          recognizer.abort(); $('orb').classList.remove('listening');
    ...
    recognizer.onerror = event => {
      if (['not-allowed', 'service-not-allowed', 'audio-capture'].includes(event.error)) {
        wakeEnabled = false; $('wake').textContent = 'Enable “Hello Orbit”'; $('status').textContent = 'Allow microphone access, then enable Hello Orbit again.';
      }
    };
    recognizer.onend = scheduleWake;
    try { recognizer.start(); } catch { scheduleWake(); }
  }
  $('wake').onclick = async () => {
    wakeEnabled = !wakeEnabled;
    const stored = await chrome.storage.local.get('wakeSites');
    const wakeSites = stored.wakeSites || {}; wakeSites[location.origin] = wakeEnabled;
    await chrome.storage.local.set({ wakeSites });
    ...
    if (wakeEnabled) { speechSynthesis.cancel(); speaking = false; startWake(); }
    else { awaitingCommand = false; clearTimeout(commandTimer); $('orb').classList.remove('listening'); }
  };
  chrome.storage.local.get('wakeSites').then(async saved => {
    if (!saved.wakeSites?.[location.origin]) return;
    try {
      const permission = await navigator.permissions.query({ name: 'microphone' });
      if (permission.state === 'granted') {
        wakeEnabled = true; $('wake').textContent = '● Hello Orbit on · click to disable'; scheduleWake();
      }
    } catch { /* User can enable it with a click when this browser requires a gesture. */ }
  });
  ...
  window.addEventListener('pagehide', () => { clearInterval(interval); wakeEnabled = false; clearTimeout(wakeTimer); clearTimeout(commandTimer); wakeRecognition?.abort(); speechSynthesis.cancel(); transcript = ''; recognition?.abort(); });
})();

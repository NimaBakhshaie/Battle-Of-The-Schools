...
const schema = {
  ...
  properties: {
    ...
    message: { type: 'string', description: 'One short sentence addressed directly to the user. For actions, use an imperative such as "Search for...", never Orbit narration such as "Searching for...".' }
  },
...
export function normalizeActionMessage(action, step = 0) {
  ...
  message = message
    ...
    .replace(/^Orbit\s+(?:will|can|is\s+going\s+to)\s+/i, '')
  ...
  const stillClaimsAgency = /\b(?:I\s*(?:['’]ll|will|['’]m|am)|we\s*(?:['’]ll|will|['’]re|are)|Orbit\s+(?:will|can|is))\b/i.test(message);
...
export async function plan({ key, task, observation, history, budget, run, signal, fetcher = fetch }) {
  ...
  if (run.tutor && data.orbitWorkspace) run.workspace = data.orbitWorkspace;

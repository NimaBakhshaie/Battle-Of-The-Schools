...
run_id = sys.argv[2]
directory = pathlib.Path('/tmp/orbit-agent')
...
export class SteelComputer {
  ...
  async ensure() {
    ...
    this.pending = this.prepare().catch(async error => {
      ...
      throw new Error('Steel is still waking. Wait briefly and retry; Orbit has not stopped or replaced the computer.');
  ...
  async prepare() {
    ...
    if (this.computer?.id) {
      ...
      this.computer = await this.api('/computers', 'POST', { name: 'Orbit current-tab agent', vcpu: 1, memoryMib: 512, timeoutSeconds: 3600, autoPause: true, idleTimeoutSeconds: 900 });
    ...
    if (!this.ready) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          result = await this.exec({ argv: ['python3', '-c', "import pathlib,base64,sys; p=pathlib.Path('/tmp/orbit-agent'); p.mkdir(mode=0o700,exist_ok=True); (p/'step.py').write_bytes(base64.b64decode(sys.argv[1]))", encoded], timeoutSeconds: 10 });
          break;
        } catch (error) {
  ...
  fetcher(key, runId) {
    return async (_url, options) => {
      ...
      const result = await this.exec({ argv: ['python3', '/tmp/orbit-agent/step.py', encoded, runId], env: { OPENAI_API_KEY: key }, timeoutSeconds: 27 }, options.signal);

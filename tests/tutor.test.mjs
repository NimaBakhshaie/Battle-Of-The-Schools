...
test('tutor highlights without clicking; only learner interaction advances; guide text stays out of snapshots', { skip: !executablePath }, async t => {
  ...
  await page.screenshot({ path: 'artifacts/orbit-guidance.png' });
  await page.locator('#search').click();
  await page.waitForFunction(() => messages.length === 1);
  assert.equal(await page.locator('#result').textContent(), 'Workshops found');
  assert.deepEqual(await page.evaluate(() => messages[0]), { type: 'ORBIT_GUIDE_RESULT', runId: 'mission', actionId: 'action-one', ok: true });
  assert.equal(await page.locator('#orbit-guidance').count(), 0);
});
...
test('Steel tutor executes a remote runtime, propagates usage and does not retry paid errors', async () => {
  ...
  let calls = 0;
  c.exec = async body => { calls++; assert.equal(body.argv[0], '/tmp/orbit-agent/venv/bin/python'); assert.equal(body.argv[2], 'plan'); assert.equal(body.env.STEEL_API_KEY, 'steel-test'); return { exitCode: 0, output: JSON.stringify({ status: 200, data: { usage: { prompt_tokens: 10 }, orbitWorkspace: { rehearsed: true } } }) }; };
  const response = await c.tutorFetcher('openai-test', 'm')('', { body: '{}' });
  assert.equal((await response.json()).orbitWorkspace.rehearsed, true); assert.equal(calls, 1);
...
test('tutor workspace keeps the original theme and renders real evidence metadata on desktop and mobile', { skip: !executablePath }, async t => {
  ...
  await page.route('https://orbit.test/**', route => {
  ...
  fs.mkdirSync('artifacts', { recursive: true });
  await page.screenshot({ path: 'artifacts/orbit-tutor-desktop.png', fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await page.screenshot({ path: 'artifacts/orbit-tutor-mobile.png', fullPage: true });

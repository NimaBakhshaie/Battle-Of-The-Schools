...
test('extension coordinates tutoring without tab actions, rejects foreign-frame acknowledgements, and verifies the next page', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    if (endpoint === 'pause') { state.run.status = 'paused'; state.run.message = 'You’re in control. Say “Hello Orbit, resume” when you’re ready.'; }
  ...
  try {
    ...
    assert.equal((await message({ type: 'ORBIT_CLAIM_SPEECH', speechId: 'run:Open the link.' })).claimed, true);
    assert.equal((await message({ type: 'ORBIT_CLAIM_SPEECH', speechId: 'run:Open the link.' })).claimed, false);
    assert.equal((await message({ type: 'ORBIT_START', task: 'Show me search' })).state.run.id, 'run');
    ...
    assert.equal((await message({ type: 'ORBIT_GUIDE_RESULT', runId: 'run', actionId: 'guide', ok: true }, 88)).ok, false);
    assert.equal(calls.filter(c => c === 'result').length, 0);
    await new Promise(r => setTimeout(r, 20));
    await message({ type: 'ORBIT_GUIDE_RESULT', runId: 'run', actionId: 'guide', ok: true });
    ...
    await message({ type: 'ORBIT_STATUS' });
    assert.ok(scripts.includes('clearGuidance'));
    guidanceWorks = false;
    await message({ type: 'ORBIT_START', task: 'Show me search again' });
    await until(() => notifications.some(item => item.error?.includes('Hello Orbit, resume')));
    const failedGuidance = notifications.findLast(item => item.error?.includes('Hello Orbit, resume'));
    assert.equal(failedGuidance.state.run.status, 'paused');
    assert.equal(values.control.status, 'paused');
    assert.match(failedGuidance.error, /Say “Hello Orbit, resume”/);

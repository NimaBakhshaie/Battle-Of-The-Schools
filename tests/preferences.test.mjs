...
test('landing settings persist only model and voice and reject foreign senders or invalid choices', async () => {
 ...
 try {
  ...
  const silent = await new Promise(resolve => internal({type:'ORBIT_SPEAK',text:'Must stay silent'},{tab:{id:12,url:'http://127.0.0.1:4318/'},url:'http://127.0.0.1:4318/'},resolve));
  ...
  assert.deepEqual(await call({type:'ORBIT_GET_PREFERENCES'}),{modelMode:'low',voiceURI:'browser-default'});
  assert.deepEqual(await call({type:'ORBIT_SET_PREFERENCES',modelMode:'high',voiceURI:'elevenlabs',token:'overwrite'}),{ok:true});
  assert.equal(values.token,'private');
  assert.deepEqual(await call({type:'ORBIT_GET_PREFERENCES'}),{modelMode:'high',voiceURI:'elevenlabs'});
  assert.ok((await call({type:'ORBIT_SET_PREFERENCES',modelMode:'invalid',voiceURI:'x'})).error);
  let replied=false;
  external({type:'ORBIT_SET_PREFERENCES',modelMode:'low',voiceURI:'x'},{url:'https://example.com/'},()=>replied=true);

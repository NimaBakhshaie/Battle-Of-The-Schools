...
test('actual coordinator serves tutor assets and rejects autonomous/unauthenticated commands', async t => {
  ...
  const child = spawn(process.execPath, ['server/index.mjs'], { env: { ...process.env, PORT: String(port), ORBIT_DATA_DIR: directory, OPENAI_API_KEY: '', STEEL_API_KEY: '' }, stdio: ['ignore', 'pipe', 'pipe'] });
  t.after(async () => {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', chunk => { if (chunk.toString().includes('Orbit is ready')) { clearTimeout(timer); resolve(); } });
    child.once('error', error => { clearTimeout(timer); reject(error); });
    child.once('exit', code => { clearTimeout(timer); reject(new Error(`Server exited ${code}`)); });
  });
  const origin = `http://127.0.0.1:${port}`;
  const html = await (await fetch(origin)).text();
  assert.match(html, /INSIDE ORBIT’S COMPUTER/);
  assert.match(await (await fetch(origin + '/tutor.css')).text(), /tutor-workspace/);
  const state = await (await fetch(origin + '/api/state')).json();
  assert.equal(state.practice, null);
  assert.equal(state.run, null);
  const headers = { 'Content-Type': 'application/json', 'X-Orbit-Request': '1' };

...
test('wake gateway failure reconciles running computer without creating a duplicate', async () => {
  const c = new SteelComputer({ key: 'test', stateFile: '/tmp/orbit-nonexistent-test-state' });
...
test('deleted saved computer is replaced exactly once', async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit-deleted-'));
  try {
...
test('failed wake never issues conflicting stop requests', async () => {
  const c = new SteelComputer({ key: 'test', stateFile: '/tmp/unused-orbit-state' });
  c.computer = { id: 'same' };

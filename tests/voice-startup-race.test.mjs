  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      window.chrome = { storage: {
        ...
        sendMessage: async message => { window.messages.push(message); if (message.type === 'ORBIT_CLAIM_SPEECH') return { claimed: true }; if (message.type === 'ORBIT_SPEAK') return { ok: true }; return { paired: true, selected: true, state: { budget: { spent: 0 }, run: null } }; },
    ...
    await page.evaluate(() => orbitMessage({ type: 'ORBIT_STATE', state: { budget: { spent: 0 }, run: { id: 'race', mode: 'local', status: 'done', message: 'Only Eric should say this.' } } }));
    await page.waitForTimeout(75);
    assert.equal(await page.evaluate(() => browserSpoken.length), 0);
    assert.equal(await page.evaluate(() => messages.some(message => message.type === 'ORBIT_SPEAK')), false);

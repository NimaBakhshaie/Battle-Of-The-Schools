...
try {
  ...
  await page.getByRole('button', { name: 'Set up Orbit' }).waitFor();
  ...
  await page.screenshot({ path: 'artifacts/orbit-desktop.png', fullPage: true });
  await page.getByRole('button', { name: 'Set up Orbit' }).click();
  assert.equal(await page.locator('#settings').evaluate(el => el.open), true);
  await page.getByRole('button', { name: 'Close settings' }).click();
  await page.setViewportSize({ width: 390, height: 844 });
  assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth));
  await page.screenshot({ path: 'artifacts/orbit-mobile.png', fullPage: true });

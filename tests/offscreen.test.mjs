...
test('a superseded ElevenLabs stream cannot start a second voice', async () => {
  ...
  const first = new Promise(resolve => listener({ target: 'orbit-offscreen', type: 'PLAY_SPEECH', text: 'old', token: 'token' }, {}, resolve));
  const second = new Promise(resolve => listener({ target: 'orbit-offscreen', type: 'PLAY_SPEECH', text: 'new', token: 'token' }, {}, resolve));
  pending[0](response());
  assert.equal((await first).ok, true);
  assert.equal(audioStarts, 0);
  assert.equal(cancelledBodies, 1);
  listener({ target: 'orbit-offscreen', type: 'CANCEL_SPEECH' }, {}, () => {});
  pending[1](response());

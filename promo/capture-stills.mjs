...
const output = path.resolve('artifacts/orbit-promo');
...
try {
  ...
  await page.evaluate(() => document.querySelector('#orbit-widget').shadowRoot.getElementById('orb').click());
  await page.waitForTimeout(250);
  await page.screenshot({ path: path.join(output, 'orbit-on-public-services.png') });
  ...
  await page.screenshot({ path: path.join(output, 'orbit-highlight.png') });
} finally {
  await browser.close();
}

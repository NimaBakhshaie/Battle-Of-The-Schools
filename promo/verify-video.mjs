...
const videoPath = path.resolve('artifacts/orbit-promo/orbit-promo.mp4');
const videoUrl = `file:///${videoPath.replaceAll('\\','/')}`;
const frameDirectory = path.resolve('artifacts/orbit-promo/qa-frames');
...
try {
  ...
  const audioStats = await page.evaluate(async () => {
    const bytes = await fetch('../artifacts/orbit-promo/orbit-promo.mp4').then(response => response.arrayBuffer());

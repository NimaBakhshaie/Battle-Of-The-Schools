...
const outputRoot = path.resolve('artifacts/orbit-promo');
...
try {
  ...
  await page.waitForFunction(() => typeof window.orbitRenderFrame === 'function');
  const stage = page.locator('#stage');
  const frameCount = fps * duration;
  for(let frame=0;frame<frameCount;frame++) {
    await page.evaluate(time => window.orbitRenderFrame(time), frame/fps);

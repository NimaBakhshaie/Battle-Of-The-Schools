...
const outputDirectory = path.resolve('artifacts/orbit-promo');
fs.mkdirSync(outputDirectory, { recursive: true });
const outputPath = path.join(outputDirectory, 'orbit-promo.webm');
...
try {
  ...
  await page.waitForFunction(() => typeof window.renderOrbitPromo === 'function');
  const downloadPromise = page.waitForEvent('download', { timeout: 120000 });
  const result = await page.evaluate(async () => {
    const recording = await window.renderOrbitPromo();
    const link = document.createElement('a');
    link.href = URL.createObjectURL(recording.blob);
    link.download = 'orbit-promo.webm';
    link.click();
